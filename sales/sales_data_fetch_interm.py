...
@ray.remote
def start_transformation(selected_file_name, working_file_path, info, source_file_path, backup_file_path):
    import sys, time
    parent_path = str(Path().resolve().parent)
    if parent_path not in sys.path:
        sys.path.insert(1, str(Path().resolve().parent))
    ...
    try:
        ...
        return {"source_file_path":source_file_path, "backup_file_path":backup_file_path, "is_transformed": True}
            ...
            if file_manager:
                ...
                if len(dataFileNames) > 0:
                    ...
                    for i in dataFileNames:
                        ...
                        result_ids.append(start_transformation.remote(selected_file_name, working_file_path, info, source_file_path,
                                                                      backup_file_path))
                    is_transformed = ray.get(result_ids)
                    for i in is_transformed:
                        if i.get('is_transformed'):
                            file_manager.move_file(i.get('source_file_path'), i.get('backup_file_path'))
                else:
